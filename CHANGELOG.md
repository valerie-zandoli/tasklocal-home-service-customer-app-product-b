...
- **A second review round caught something more serious**: the `created_at` trigger fix above had been committed and its live test was passing in this repo's own CI, but the migration itself was never actually run against the shared database — a genuine "the code is right but production still has the bug" gap, not a doc mismatch. Found live (a real backdate PATCH succeeded when it should have been rejected), applied, and re-verified (21/21 Tier 2 tests passing against production). Same review found `backend/seed_demo_bookings.sql` had likewise never been run live — one demo account (Morgan Reyes) had zero bookings, contradicting the login screen's "all four pre-loaded" claim. Applied; all four accounts now have real, varied booking history.
