...
- Shared pure logic (HTML-escaping, listing filters, currency formatting via `Intl.NumberFormat`, deciding whether a booking shows a rating form) lives in `frontend/js/utils.js` rather than being copy-pasted per page, and is unit-tested in `frontend/js/utils.test.mjs`. `frontend/js/nav.js`'s actual DOM manipulation (active-link marking, HTML-escaping in the rendered markup, the logout redirect) is separately tested in `frontend/js/nav.test.mjs` using `jsdom` — the one place in the suite that needs a real `document`.
...
- `.github/workflows/test.yml` runs `node --test frontend/js/*.test.mjs` on every push/PR (`frontend/` needs `npm install` first now, for `jsdom` — see Section 4 below).
...
```
`npm install` is only needed once, for `jsdom` — `nav.test.mjs`'s DOM-level tests are the one place in the suite that need a real `document`/`window`/`sessionStorage` (`renderNav()`'s active-link marking and HTML-escaping, `requireSession()`'s redirect-vs-return branching). Everything else (pricing/booking-id logic, `escapeHtml`/`filterListings`/`getRatingDisplayState`) is still plain `node --test` with zero dependencies (Node 20+; see the note at the top of each test file). This does **not** cover `backend/schema.sql`'s RLS policies, triggers, or functions — those need a real Supabase project (see "Known limitations" above).
