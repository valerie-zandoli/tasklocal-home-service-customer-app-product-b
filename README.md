...
- `.github/workflows/test.yml` has a `deploy-preview` job (`needs: test`) building toward CI-gated deployment, deliberately targeting a **preview** URL, not production — read the comment directly above that job in the workflow file before changing that. A first attempt at this used `--prod` directly and broke the live site: `vercel build` + `vercel deploy --prebuilt` (the officially documented CI deploy pattern) generates a different routing config than Vercel's own managed build pipeline, and that difference 404'd the root path (`/`) for real visitors for several minutes — restored at the time by reverting to a direct `vercel --prod` deploy, not by understanding the actual cause. The cause above is now understood and fixed, and verified safe on preview — but the `deploy-preview` job still needs to actually run clean a few times for real, on real pushes, before deliberately flipping it to `--prod` and disabling Vercel's Git-integration auto-deploy (Settings → Git → Ignored Build Step). Until then, Vercel's own auto-deploy remains the only thing pushing to production.
...
- Vercel doesn't auto-prune deployments, and `deploy-preview` creates a new one on every push — 31 had piled up (almost entirely superseded) before anyone noticed. `scripts/cleanup-deployments.sh` removes every deployment for this project except whichever one currently holds the production alias (`--safe`, so the live site is never at risk regardless of when this runs). Not on a schedule — run it occasionally by hand.
