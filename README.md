...
- `.github/workflows/test.yml` has a `deploy-preview` job (`needs: test`) building toward CI-gated deployment, but it deliberately targets a **preview** URL, not production, right now — read the comment directly above that job in the workflow file before changing that. A first attempt at this used `--prod` directly and broke the live site: `vercel build` + `vercel deploy --prebuilt` (the officially documented CI deploy pattern) generates a different routing config than Vercel's own managed build pipeline, and that difference 404'd the root path (`/`) for real visitors for several minutes — restored by reverting to a direct `vercel --prod` deploy, not by finding the actual root cause. `node --test` didn't catch it because nothing tests the deployed site itself.
  **Resuming this:** not on a calendar date — the condition is a run of consecutive clean `deploy-preview` runs *and* actually understanding why the root path broke (not just confirming it stopped happening), before deliberately flipping the job back to `--prod` and disabling Vercel's Git-integration auto-deploy (Settings → Git → Ignored Build Step) in its dashboard. Until then, Vercel's own auto-deploy remains the only thing pushing to production, and it's untouched.
