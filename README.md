...
- `frontend/manifest.json` (linked from every page) makes the app installable — "Add to Home Screen" / standalone window — with `assets/logo.svg` plus 192×192 and 512×512 PNG fallbacks (Chrome's install criteria specifically want a PNG, not just an SVG marked `sizes="any"`).
- `vercel.json` sets a strict Content-Security-Policy (`script-src`/`style-src 'self'`, no `'unsafe-inline'` — the app has zero inline `<script>` or `style="..."` left anywhere, on purpose) plus `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, and a `Permissions-Policy` disabling camera/mic/geolocation. **This only takes effect on an actual Vercel deployment** — it's not read by local demo mode's `python3 -m http.server`, and there's no live deployment to verify it against (see Known Limitations).
