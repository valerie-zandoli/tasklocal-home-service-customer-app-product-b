...
- `backend/schema.sql`'s RLS policies, triggers, and functions themselves (the actual Postgres-level enforcement — RLS denying cross-customer access, the triggers rejecting a tampered update) have never been exercised against a real database — there's no live Supabase project for this presentation. That specific gap is real and stays open. What *is* now covered, without a real backend: `frontend/js/api-supabase.test.mjs` exercises every one of `api.js`'s real-Supabase-mode code paths (auth, PostgREST query building, the `create_booking_with_schedule` retry-on-collision logic, response normalization) against a hand-built fake client matching the real one's shape — real application logic, verified offline, just not the database-level policies underneath it. The full suite (`node --test --experimental-test-coverage frontend/js/*.test.mjs`) is 89 tests across every page module, both app modes, data integrity, WCAG contrast, and axe-core accessibility checks — see Section 4 below. If this project is ever deployed for real, run the "Real Supabase mode only" checklist item below first regardless.
